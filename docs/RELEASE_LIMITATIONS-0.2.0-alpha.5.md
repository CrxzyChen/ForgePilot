# AI Game Studio 0.2.0 Alpha 5 limitations
...
- Prefab instances produced by the semantic authoring API use the built-in
  ...
  engine-owned marker.
- Empty projects default to a 64 MiB QuickJS memory budget so medium gameplay
  scenarios can retain useful per-Tick snapshots and Timeline evidence. Very
  long trace-heavy sessions still need an explicit project budget or shorter
  bounded runs.
...
- Engine-level third-party native capability loading is not available. Project
  ...
  engine changes.
- Copilot persists Goal state and the most recent execution Plan. Starting a
  Goal-mode turn resumes an existing paused, blocked, or usage-limited Goal;
  Stop interrupts and pauses it, while Remove clears it.
- The native player forwards held movement and action inputs on every current
  Tick, maps R and Escape for project Commands, and reserves Q for quitting.

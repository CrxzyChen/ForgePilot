# AI Game Studio 0.2.0 Alpha 3 limitations
...
- Engine-level third-party native capability loading is not available. Project
  ...
  engine changes.
- Copilot persists Goal state and the most recent execution Plan. When the Codex
  runtime cannot emit structured Plan events, Studio parses the explicit
  `STUDIO_PLAN` contract from the assistant response. Plan progress is therefore
  reported by the agent rather than independently inferred by Studio.
...
- Automated installed-build and UI-operator checks pass. Independent Tank
  project completion is the active Alpha 3 acceptance workload; the dashboard

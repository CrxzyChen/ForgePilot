...
5. Use typed payload Schemas, stable IDs, fixed Tick, `context.randomU32()`, and queued Events. Runtime objects must use explicit stable IDs; create or remove them with `context.spawn` / `context.destroy`, toggle them with `context.setEnabled` / `context.setVisible`, and switch only to packaged Scene paths or IDs with `context.loadScene`. Lifecycle changes apply deterministically in `engine:post-update`.
6. Emit versioned audio through `context.playAudio`, `stopAudio`, `pauseAudio`, `resumeAudio`, and `setAudioBus`; use only ready asset IDs and declared buses. Route keyboard, pointer, and UI through semantic actions from `input/actions.json`.
7. Never use wall clock, ambient random, filesystem, network, process, dynamic native modules, synthetic collisions, or reentrant Event delivery.
8. Present the complete ChangeSet and expected effects before approval.
9. After approval, apply atomically and add or update a deterministic replay with per-Tick state hashes where relevant.
10. Run `runtime.run`, `runtime.read_trace`, `replay.run`, and the affected tests. Inspect Event Timeline, System trace, lifecycle operations, physics contacts, audio events, active Scene, watches, budgets, and structured source diagnostics; roll back if invalid.

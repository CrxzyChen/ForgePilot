...
5. Use typed payload Schemas, stable IDs, fixed Tick, `context.randomU32()`, and queued Events. Never use wall clock, ambient random, filesystem, network, process, dynamic native modules, or reentrant Event delivery.
6. Present the complete ChangeSet and expected effects before approval.
7. After approval, apply atomically and add or update a deterministic replay with per-Tick state hashes where relevant.
8. Run `runtime.run`, `runtime.read_trace`, `replay.run`, and the affected tests. Inspect Event Timeline, System trace, watches, budgets, and structured source diagnostics; roll back if invalid.
